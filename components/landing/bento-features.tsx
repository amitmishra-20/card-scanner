      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:grid grid-cols-3 gap-4">
          ...
          >
            <div>
              ...
              <p className="text-sm text-muted-foreground max-w-xs">
                Our vision model reads handwritten notes, obscure fonts, and
                damaged cards.
              </p>
            </div>
            <div className="mt-8 space-y-2">
              {["Printed text", "Handwriting", "Damaged cards", "QR codes"].map(
                (item) => (
                  <div
                    key={item}
          ...
          {features.map((f, i) => (
            <AnimatedCard
              key={f.title}
              className={`glass p-6 flex flex-col justify-center hover:border-white/10 hover:bg-[oklch(0.10 0 0)] transition-all duration-300 cursor-default animate-delay-${(i + 1) * 100}`}
          ...
          {/* Wide bottom cell */}
          <AnimatedCard
            className="col-span-2 glass p-6 overflow-hidden hover:border-white/10 hover:bg-[oklch(0.10 0 0)] transition-all duration-300 cursor-default animate-delay-500"
        ...
        <div className="md:hidden space-y-4">
          <div className="glass p-8 hover:border-white/10 hover:bg-[oklch(0.10 0 0)] transition-all duration-300">
            ...
            <p className="text-sm text-muted-foreground">
              Reads any card, any font, any condition.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {features.map((f) => (
              <div
                key={f.title}
                className="glass p-5 hover:border-white/10 hover:bg-[oklch(0.10 0 0)] transition-all duration-300"
              >
          ...
          <div className="bg-[oklch(0.08 0 0)] border border-white/5 p-5 hover:border-white/10 hover:bg-[oklch(0.10 0 0)] transition-all duration-300">
            ...
            <div className="flex gap-2 overflow-hidden">
              {["Design Co.", "Tech Labs", "Global Inc.", "Startup HQ"].map(
                (c) => (
                  <div
                    key={c}
                    className="shrink-0 bg-white/[0.03] border border-white/5 px-3 py-2 text-xs text-white/50"
                  >
                    {c}
                  </div>
                )
